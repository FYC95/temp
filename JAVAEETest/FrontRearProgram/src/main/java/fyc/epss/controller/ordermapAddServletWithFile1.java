...
//该Serlet的目的是添加商家
@WebServlet(urlPatterns = "/ordermap/addOneWithFile1.do")
public class ordermapAddServletWithFile1 extends HttpServlet {
...
//                    continue;
        ...
        Integer businessExplain = Integer.valueOf(aList.get("businessExplain"));


        orderMap info = new orderMap(businessName,businessExplain);
        //调用service执行查询业务
        orderMapServiceImpl ordermapService = new orderMapServiceImpl();
        Boolean num = ordermapService.addOne(info);
        //封装查询结果
        DataFlexPacking mad = null;
        if (!num)
            mad = DataFlexPacking.fail("添加订单信息失败");
        else
            mad = DataFlexPacking.success("添加订单信息成功");
        //转换为JSON
