...
//该Serlet的目的是添加商家
@WebServlet(urlPatterns = "/food/addOneWithFile1.do")
public class foodAddServletWithFile1 extends HttpServlet {
...
//                    continue;
        ...
        Integer password = Integer.valueOf(aList.get("password"));
        Double businessExplain = Double.valueOf(aList.get("businessExplain"));
        String businessAddress = filename;


        food food = new food(businessName,businessAddress,businessExplain,password);
        //调用service执行查询业务
        foodServiceImpl foodService = new foodServiceImpl();
        Boolean num = foodService.addOne(food);
        //封装查询结果
        DataFlexPacking mad = null;
        if (!num)
            mad = DataFlexPacking.fail("添加食品数据失败");
        else
            mad = DataFlexPacking.success("添加食品数据成功");
        //转换为JSON
